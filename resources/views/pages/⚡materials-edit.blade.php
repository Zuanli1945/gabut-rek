...
new #[Title('Edit Material')] #[Layout('layouts.app')] class extends Component {
    ...
    {
        return \App\Models\Formula::whereHas('materials', fn($q) => $q->where('material_id', $this->materialId))
            ->get(['id', 'nama_formula', 'jenis_konsentrasi']);
    }

    public function mount(int $id): void
    {
        $material = Material::with('subCategory')->findOrFail($id);
        $this->materialId = $material->id;
        $this->nama = $material->nama;
        $this->tipe = $material->tipe;
        $this->subCategoryId = $material->sub_category_id;
        $this->subCategoryName = $material->subCategory?->name;
        $this->scent_family = $material->scent_family;
        $this->harga_beli = (float) $material->harga_beli;
        $this->jumlah_beli = (float) $material->jumlah_beli;
        $this->satuan = $material->satuan;
        $this->stock_saat_ini = (float) $material->stock_saat_ini;
...
<div class="p-6 lg:p-8">
    <div class="mb-8">
        ...
        <h1 class="mt-2 font-display text-3xl font-medium text-[var(--ink)]">Edit Material</h1>
        <p class="mt-1 font-body text-sm text-[var(--ink-muted)]">Update material details</p>
    </div>

    <form wire:submit="save" class="card max-w-xl space-y-6">
        <div>
            <label class="micro-label mb-1 block">Material Name</label>
            <input type="text" wire:model="nama" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)]" />
        ...
        <div>
            ...
            <input type="text" wire:model="scent_family" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)]" />
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <div>
                ...
                <input type="number" wire:model="harga_beli" step="0.01" min="0" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)] tabular-nums" />
            ...
            <div>
                ...
                <input type="number" wire:model="jumlah_beli" step="0.01" min="0.01" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)] tabular-nums" />
        ...
        <div>
            ...
            <input type="number" wire:model="stock_saat_ini" step="0.01" min="0" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)] tabular-nums" />
        ...
        @php $usedIn = $this->used_in_formulas; @endphp
        @if($usedIn->isNotEmpty())
            <div class="card max-w-xl mt-6">
                <p class="micro-label mb-3">Used in {{ $usedIn->count() }} Formula(s)</p>
                <div class="space-y-2">
                    @foreach($usedIn as $f)
                        <a href="{{ route('formulas.show', $f->id) }}" class="flex items-center justify-between py-1.5 border-t border-[var(--border-soft)] first:border-t-0 hover:bg-[var(--paper)] -mx-3 px-3 rounded-[2px] no-underline">
                            <span class="font-body text-sm text-[var(--ink)]">{{ $f->nama_formula }}</span>
                            @if($f->jenis_konsentrasi)
                                <span class="badge-amber text-[10px]">{{ $f->jenis_konsentrasi->value }}</span>
                            @endif
                        </a>
                    @endforeach
                </div>
        ...
        <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" class="btn-amber text-sm">Update Material</button>
        </div>
    </form>
</div>
