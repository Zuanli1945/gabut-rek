...
new #[Title("Edit Product")] #[Layout("layouts.app")] class extends Component {
    ...
    public function mount(int $id): void
    {
        ...
        $produk = Produk::with("formulas")->findOrFail($id);
        $this->nama_produk = $produk->nama_produk;
        $this->stock = (float) $produk->stock;
        $this->harga_jual = (float) $produk->harga_jual;

        $this->formulaJson = $produk->formulas
            ->map(
                fn($f) => [
                    "formula_id" => $f->id,
                    "jumlah_ml" => (float) $f->pivot->jumlah_ml,
                    "persentase_komposisi" =>
                        (float) $f->pivot->persentase_komposisi,
                ],
            )
            ->toJson();
    ...
    public function save(): void
    {
        ...
        $produk = Produk::findOrFail($this->productId);
...
<div class="p-6 lg:p-8"
    ...
    <div class="mb-8">
        ...
        <h1 class="mt-2 font-display text-3xl font-medium text-[var(--ink)]">Edit Product</h1>
        <p class="mt-1 font-body text-sm text-[var(--ink-muted)]">Update product and its formula composition</p>
    </div>
    ...
    <form x-on:submit.prevent="submitForm()" class="max-w-2xl space-y-6">
        ...
        <div class="card space-y-4">
            ...
            <div>
                ...
                <input type="text" wire:model="nama_produk" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)]" />
            ...
            <div>
                ...
                <input type="number" wire:model="stock" step="0.01" min="0" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)] tabular-nums" />
            ...
            <div>
                ...
                <input type="number" wire:model="harga_jual" step="1" min="0" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)] tabular-nums" />
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="btn-amber text-sm">Update Product</button>
        </div>
    </form>
</div>
