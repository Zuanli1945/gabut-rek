...
new #[Title("New Formula")] #[Layout("layouts.app")] class extends Component {
    ...
    public function save(): void
    {
        ...
        // Server-side cross-validation: gram-derived % vs manual %
        $totalGram = collect($materials)->sum(fn($m) => (float) ($m["gram"] ?? 0));
        if ($totalGram > 0) {
            foreach ($materials as $mat) {
                $gram = (float) ($mat["gram"] ?? 0);
                if ($gram <= 0) {
                    continue;
                }
                $computedPct = ($gram / $totalGram) * 100;
                $diff = abs($computedPct - (float) $mat["persentase"]);
                if ($diff > 1) {
                    $opt = collect($this->materialOptions)->firstWhere("id", $mat["material_id"]);
                    $name = $opt["nama"] ?? $mat["material_id"];
                    session()->flash(
                        "warning",
                        "Material \"{$name}\": gram-derived % ({$computedPct}%) differs from manual % ({$mat["persentase"]}%) by {$diff}%."
                    );
                    break;
                }
            }
...
<div class="p-6 lg:p-8"
    ...
    <div class="mb-8">
        ...
        <h1 class="mt-2 font-display text-3xl font-medium text-[var(--ink)]">New Formula</h1>
        <p class="mt-1 font-body text-sm text-[var(--ink-muted)]">Create a fragrance formula and calculate material cost</p>
    </div>
    ...
    <form x-on:submit.prevent="submitForm()" class="max-w-2xl space-y-6">
        ...
        <div class="card space-y-4">
            ...
            <div>
                ...
                <input type="text" wire:model="nama_formula" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)]" placeholder="e.g. Yuzu Matcha Milk" />
            ...
            <div>
                ...
                <textarea wire:model="deskripsi" rows="2" class="w-full rounded-[2px] border border-[var(--border-hair)] bg-[var(--cream)] px-3 py-2 font-body text-sm text-[var(--ink)] outline-none focus:ring-2 focus:ring-[var(--amber)]" placeholder="e.g. A fresh and green scent with citrus and matcha notes..."></textarea>
        ...
        <div class="card">
            ...
            <div class="flex flex-col items-center">
                <svg viewBox="0 0 280 220" class="w-full max-w-[240px] h-auto">
                    ...
                    {{-- Top --}}
                    <polygon points="140,10 75,90 205,90" fill="url(#topGrad)" />
                    ...
                    {{-- Mid --}}
                    <polygon points="75,90 45,150 235,150 205,90" fill="url(#midGrad)" />
                    ...
                    {{-- Base --}}
                    <polygon points="45,150 20,205 260,205 235,150" fill="url(#baseGrad)" />
        ...
        <div class="card" x-show="materials.length > 0">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <p class="font-display text-lg font-medium text-[var(--amber)] tabular-nums">Rp<span x-text="getHppFormatted()"></span>/ml</p>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="btn-amber text-sm">Save Formula</button>
        </div>
    </form>
</div>
